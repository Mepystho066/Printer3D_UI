    ...
    def body(self):
        with dpg.group(horizontal=True):
            with dpg.group(tag="GupoTiempo",horizontal=True):
                Horas = dpg.add_slider_int(label="Horas",tag="Horas",vertical=True,default_value=1,max_value=72,width=25,callback = Cost)
                Minutos = dpg.add_slider_int(label="Minutos",tag="Minutos",vertical=True,default_value= 1,max_value=59,width=25,callback = Cost)
